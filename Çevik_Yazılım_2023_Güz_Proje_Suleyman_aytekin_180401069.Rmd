...
Grafikte görüldüğü üzere, yaş sayısı azaldıkça alınan süreler de azalıyor. Bu durumda 500 dakika ve üzeri süre alan futbolcuların aranması gerekiyor, bu da arama yelpazemizi istediğimiz şekilde daraltıyor.
...
Yaptığımız grafik istatistikleri ve diğer özelliklere dayanarak, aradığımız defans oyuncusunun en az 10 maçta ilk 11 başlamış olması, pas denemesi 50 ve üzeri, uzun pas başarı oranının %50 ve üzeri olması, 22 yaş ve altındaki oyuncuların aranmasına kulüple yapılan istişareler sonucunda karar kılınmıştır.

## Kriterlere uyan oyuncuların tablosu

```{r echo=FALSE, message=FALSE, warning=FALSE}
library(dplyr)

# Filtreli veriyi oluştur
filtreli_veri <- myDF %>%
  filter(Pos == "DF", Starts >= 10, PasTotAtt >= 50, PasLonCmp. >= 50, PasTotCmp. >= 75, Age <= 22, Min >= 500)

# Tabloyu oluştur
print(filtreli_veri)
```
## Genel istatislik verileri
Tabloda görüldüğü üzere 28 oyuncu sıralanmıştır.

```{r echo=FALSE, message=FALSE, warning=FALSE}
# Oyuncu sayısını ve toplam satır sayısını bul
oyuncu_sayisi <- nrow(filtreli_veri)
toplam_satir_sayisi <- nrow(myDF)

# Oranı hesapla
oran <- (oyuncu_sayisi / toplam_satir_sayisi) * 100

# Sonucu yazdır
cat("Oyuncu Sayısı:", oyuncu_sayisi, "\n")
cat("Toplam Satır Sayısı:", toplam_satir_sayisi, "\n")
cat("Oyuncu Oranı: %", oran, "\n")
```

Sonuç olarak, sadece 28 futbolcu bulunmaktadır ve bu sonuç, istatistiksel olarak veri setimizdeki bütün oyuncuların yaklaşık %1'ine denk gelmektedir.

```{r echo=FALSE, message=FALSE, warning=FALSE}
# Filtrelenmiş veri setindeki ortalama yaş ve süreleri hesapla

ortalamalar <- filtreli_veri %>%
  summarise(
    Ortalama_Starts = mean(Starts),
    Ortalama_PasTotAtt = mean(PasTotAtt),
    Ortalama_PasLonCmp = mean(PasLonCmp.),
    Ortalama_PasTotCmp = mean(PasTotCmp.),
    Ortalama_yas = mean(Age),
    Ortalama_sure = mean(Min)
  )
...
Bu istatistikler, belirlenen kriterlere uyan 28 futbolcunun genel profilini ortaya koymaktadır.
...
# Yukarıdaki "results='hide'" ifadesi kullanıldığından dolayı çıktı görüntülenmeyecektir. Eğer render işlemi başarılı olduysa testleri geçmiş demektir.
# Testlerin detaylı geçtiğini görmek için "results='hide'" ifadesini kaldırın.



# 180401069_Çevik_yazılım_test_kodları_Suleyman_aytekin.R dosyasını çağır
source("180401069_Çevik_yazılım_test_kodları_Suleyman_aytekin.R")

# Tüm testleri çalıştıran kod
test_result <- test_file("180401069_Çevik_yazılım_test_kodları_Suleyman_aytekin.R")
summary(test_result)
print(test_result)
```
...
Projede genel olarak, veri setinden veriler analiz edilmiş, gerekli veriler çekilip, görselleştirilmiş, istatistikleri alınarak yazılmış bir rapor oluşturulmuştur. Sonuç olarak, Beşiktaş JK futbol kulübünün kriterlerine uygun 28 adet defans oyuncusu bulunmuş ve başarıyla sonuçlanmıştır. Oluşan problem büyük ölçüde çözülmüş olup, hem firmamız hem de kulüp sonuçlardan memnun olarak ayrılmıştır. Sonuçlar Beşiktaş JK futbol kulübüne teslim edilmiştir.

Kulüp, elde edilen futbolcuları fiziksel olarak gözlemleyerek aralarından en uygun olanı seçmesi için kulübe bırakılmıştır. Analizimiz gayet yerinde olup, 2700 yakın futbolcu arasından aranan kritere uygun 28 futbolcuya ulaşılmıştır. Eksik yönümüz ise futbolcunun güncel piyasa değeridir. İleride bu projeyi geliştirmek için yapılması gerekenler şunlardır: futbolcu skalasını büyütmek, ilk beş lig dışından futbolcuların verisine ulaşmak, futbolcu piyasasını eklemek ve bunlar üzerinden işlem yapmaktır.